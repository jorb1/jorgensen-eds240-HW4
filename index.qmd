...
![](data/final_viz_lonnnnggg.png){fig-alt="An infographic, with big bold white letters on a black background, describing how while it is possible to see kelp canopy from space, the variability of the kelp by season necessitates seasonal averaging for analyzing broader trends in coverage over time, and if working with maximum extent of coverage, summer is likely the season being observed."}
